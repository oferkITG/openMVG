...
if __name__ == "__main__":
    ...
    search_db(db=db_vec, db_offset=scenario.exit_start_frame, target_encoding=target_encoding, label='after_{0:d}'.format(scenario.exit_start_frame), results_path=results_path)
    ...
    #
    # # search DB
    # db_offset = scenario.exit_start_frame
    # search_db = db_vec[db_offset:]
    # similarity = torch.matmul(search_db, torch.unsqueeze(target_encoding, 0).transpose(0, 1)).squeeze() / ((torch.norm(search_db, dim=1)) * torch.norm(target_encoding))
    # plt.figure()
    # plt.plot(np.arange(scenario.exit_start_frame, db_offset + int(search_db.shape[0])), np.array(similarity))
    # plt.xlabel('frames')
    # plt.ylabel('similarity')
    # plt.grid()
    # plt.savefig(results_path / 'similarity_{0:d}.png'.format(TARGET_FRAME))
    #
    # N_ROWS: int = 3
    # N_COLS: int = 3
    # CENTER_PLOT_INDICES = ((N_ROWS - 1) // 2, (N_COLS - 1) // 2)
    ...
    #             axes[row, col].set_title("{0:d}(similarity={1:.2f})".format(int(knn_indices[k]), float(knn_scores[k])))
    #             k += 1
    # plt.savefig(results_path / 'knn_{0:d}_entire_db.png'.format(TARGET_FRAME))
