...
encoder_type = "eigenplaces" # change here to "cosplace" if you'd like
device_id = "cuda:0"
...
class VprEncoderForTests:
    vpr_encoder: GeoLocalizationNet
    OUTPUT_DIM: int = 2048
    BACKBONE = 'ResNet50'
    def __init__(self):
        self.vpr_encoder = GeoLocalizationNet(backbone=self.BACKBONE, fc_output_dim=self.OUTPUT_DIM).to(device)

    def encode_img(self, img: np.ndarray):
        '''
