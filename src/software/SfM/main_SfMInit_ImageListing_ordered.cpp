...
int main(int argc, char **argv)
{
  ...
  std::vector<std::string> vec_image_tmp = stlplus::folder_files( sImageDir );
  //std::sort(vec_image.begin(), vec_image.end());
  int images_len = (int)vec_image_tmp.size();
  std::vector<std::string> vec_image;
  for(int i=0; i<images_len ; i++) {
    std::string image_name = "frame" + std::to_string(i) + ".jpg";
    vec_image.push_back(image_name);
  }
